...
//                     label="Status"
//                     className="mb-4 text-amber-50"
//                     {...register("status", { required: true })}
//                 />
//                 <Button type="submit" bgColor={post ? "bg-yellow-500" : undefined} className="w-full">
//                     {post ? "Update" : "Submit"}
//                 </Button>
//             </div>
//         </form>
...
export default function PostForm({ post }) {
  ...
  // Fetch featured image preview for existing post
  useEffect(() => {
    if (post?.featuredImage) {
      async function fetchPreview() {
        try {
          const url = await fileService.filePreview(post.featuredImage);
          setPreviewUrl(url);
        } catch (err) {
